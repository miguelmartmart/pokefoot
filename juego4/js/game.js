...
function updateBall() {
  ...
  if (currentTurn === "pikachu" && ball.x >= goalLineRight - ball.radius) {
    ...
    if (willFail) {
      ...
      document.getElementById("goalMessage").innerText = "⚽ ¡GOOOL de Pikachu!";
...
function kick() {
  ...
  if (currentTurn === "pikachu") {
    ...
    ball.speedX = 6;      // hacia la derecha
...
function gameLoop() {
  drawBackground();
  drawPikachu();
  drawArceus(); // ← ahora ya está definida correctamente
  drawBall();
  drawEffect();
  updateBall();
...
// 🎵 inicia música con primer clic del usuario (autoplay workaround)
  document.body.addEventListener("click", () => {
    backgroundMusic.play().catch(err => console.warn("Autoplay bloqueado:", err));
  }, { once: true });
